"""
PlantWijs dataset builder — v1.5
• Fix: IVs-sheet met twee header-rijen ("LIGHT"/"Average") → leest nu met header=[0,1] en flatteert naar 'light_average', 'moisture_average', ...
• Fix: eerdere 'map is not subscriptable' blijft verholpen
• CSV-reader blijft robuust (UTF‑8/UTF‑16; ; , \t of whitespace)
• Extra logging zodat we precies zien wat er gemapt is

Gebruik
  cd C:\\PlantWijs
  venv\\Scripts\\python build_dataset.py

Benodigde inputbestanden (./data):
- verspreidingsatlas_planten.csv   # NDFF soortenlijst
- ellenberg.xlsx                   # Tichý et al. IVs (tabblad met kolommen LIGHT/TEMPERATURE/MOISTURE/REACTION/NUTRIENTS/SALINITY)
...
def load_species_csv(path: str) -> pd.DataFrame:
    ...
    col_status = pick_first(["status","herkomst","inheems","inheems_of_niet","soortstatus","oorspronkelijk"])  # optional
    ...
    if col_status: ren[col_status] = "_status"
    df = df.rename(columns=ren)

    if "_status" in df.columns:
        mask = df["_status"].astype(str).str.contains("inheems", case=False, na=False)
        df = df.loc[mask].copy()
...
def load_ellenberg_xlsx(path: str) -> pd.DataFrame:
    ...
    # Sorteer voorkeur: expliciet IVs/Tichy, dan alles
    ...
    # Probeer expliciet twee header-rijen eerst (zoals in jouw screenshot)
    for sheet in sheet_candidates:
        for header in ([0,1],[1,2],1,0,2):
            try:
                df_try = xl.parse(sheet, header=header)  # header kan list[int] zijn → MultiIndex
                df_try.columns = _flatten(df_try.columns)
                if not df_try.empty:
                    parsed = (sheet, (header if isinstance(header, tuple) else ((header,) if isinstance(header,int) else tuple(header))), df_try)
                    break
            except Exception:
                continue
    ...
    if not col_tax:
        # Heuristiek: kolom met binomiale namen
    ...
    # L/F/N/R/S zoeken — include 'light_average' etc.
    def pick_metric(key: str, variants: List[str]) -> Optional[str]:
        col = _find_col([fr"^{key}$", fr"^ellenberg_{key}$"])  # korte code
        if col:
            return col
        return _find_col(variants)
