r"""
PlantWijs API — v3.9.2 (Gt 1–19 tekstlegenda)
===================================================
Wijzigingen t.o.v. v3.9.1:
- UI: Tekstlegenda voor Grondwatertrappen (Gt) met echte index→klasse mapping.
- Geen functionele wijzigingen aan data/filters. Alles werkt zoals 3.9.1.
...
def _vochtklasse_from_gt_code(gt: Optional[str]) -> Optional[str]:
    """Zet BRO-Gt code om naar onze vochtklassen.
    Ondersteunt:
      - ordinale codes '1'..'19' (Ia..VIIId, 1=zeer nat → 19=zeer droog)
      - Romeinse basiscodes ('I','II','III','IV','V','VI','VII','VIII') incl. suffixen (a/b/c/o/d/u)
    """
    if not gt:
        return None
    s = str(gt).strip()

    # 1) Ordinale Gt-code 1..19 (Ia..VIIId)
    if s.isdigit():
        v = int(s)
    ...
    # 2) Romeinse code met optionele letter (bv. 'IVc', 'VIIo')
    s_up = s.upper()
...
def vocht_from_gwt(lat: float, lon: float) -> Tuple[Optional[str], dict, Optional[str]]:
    ...
    gt_code: Optional[str] = None  # bv. 'IVc' of '8'
    # Probeer eerst directe property-namen
    for k in ("gt", "grondwatertrap", "GT", "Gt"):
        if k in props and props[k]:
            gt_code = str(props[k]).strip()
            break
    # Zo niet, lees plaintext of value_list
    if not gt_code and "_text" in props:
        kv = _parse_kv_text(props["_text"])
        for k in ("gt", "grondwatertrap", "GT"):
            if k in kv and kv[k]:
                gt_code = str(kv[k]).strip()
                break
        if not gt_code:
            m = re.search(r"\bGT\s*([IVX]+[a-z]?)\b", props["_text"], re.I)
            if m:
                gt_code = m.group(1).strip()

    # Ordinale code uit value_list (1..19)
    if not gt_code and "value_list" in props and str(props["value_list"]).strip():
        gt_code = str(props["value_list"]).strip()

    klass = _vochtklasse_from_gt_code(gt_code)

    # Fallback: GLG/GHG (alleen als klass nog None is)
    if not klass:
        for key in ("glg", "ghg"):
            ...
            if depth is not None:
                ...
                return klass, p2, gt_code

    return klass, props, gt_code
...
def advies_geo(
    ...
    # Géén default op vocht als het onbekend is
...
<head>
  ...
  <style>
    :root { --bg:#0b1321; --panel:#0f192e; --muted:#9aa4b2; --fg:#e6edf3; --line:#1c2a42; }
    * { box-sizing:border-box; }
    html, body { height:100%; }
    body { margin:0; height:100%; overflow:hidden; font:14px/1.5 system-ui, -apple-system, Segoe UI, Roboto, Arial; background:var(--bg); color:var(--fg); }
    header { padding:10px 14px; border-bottom:1px solid var(--line); position:sticky; top:0; background:var(--bg); z-index:10; display:flex; gap:10px; flex-wrap:wrap; align-items:center; }
    header h1 { margin:0; font-size:18px; }
    .wrap { display:grid; grid-template-columns: minmax(520px, 60%) 1fr; gap:12px; padding:12px; height: calc(100vh - 58px); }
    #map { height:100%; border-radius:12px; border:1px solid var(--line); box-shadow:0 0 0 1px rgba(255,255,255,.05) inset; }
    .panel { background:var(--panel); border:1px solid var(--line); border-radius:12px; padding:12px; }
    .right { display:flex; flex-direction:column; min-height:0; } /* min-height:0 is key for flex+overflow */
    ...
    .toprow { display:flex; justify-content:space-between; align-items:center; gap:8px; margin-bottom:8px; }
    .tblwrap { flex:1; min-height:0; overflow:auto; border:1px solid var(--line); border-radius:10px; }
    table { width:100%; border-collapse:collapse; }
    th, td { padding:8px 10px; border-bottom:1px solid #182742; }
    th { text-align:left; color:#b0b8c6; position:sticky; top:0; background:var(--panel); z-index:1; }

    /* Kaart-info control (rechtsboven op de kaart) */
    .infoctl { background:rgba(11,18,38,.92); color:var(--fg); border:1px solid var(--line); border-radius:12px; padding:10px 12px; box-shadow:0 6px 18px rgba(0,0,0,.25); min-width:240px; }
    .infoctl .title { font-weight:600; margin-bottom:6px; }
    .infoctl .row { display:flex; gap:6px; }
    .infoctl .k { color:#b0b8c6; width:48px; flex:0 0 48px; }
    .infoctl .v { flex:1; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
    .infoctl .muted { color:#9aa4b2; font-size:12px; margin-top:4px; }
...
<body>
  ...
  <div class="wrap">
    ...
    <div class="right">
      <div class="panel">
        <div class="muted">Actieve filters:</div>
        <div id="chips" class="chips"></div>
        <div class="toprow">
          <div class="muted" id="count"></div>
          <button id="btnRefresh" class="btn">Vernieuw</button>
        </div>
        <div class="tblwrap">
          <table id="tbl">
            <thead>
              <tr>
                ...
                <th style="width:28%">Wetenschappelijke naam</th>
                <th style="width:12%">Licht</th>
                <th style="width:12%">Vocht</th>
                <th style="width:12%">Bodem</th>
                <th style="width:10%">WHZ</th>
              </tr>
            </thead>
            <tbody></tbody>
          </table>
  ...
  <script>
    ...
    // Overlays (blijven gewoon bestaan)
    let overlays = {};
    async function loadWms(){
      ...
      L.control.layers({ 'OSM': base }, overlays, { collapsed:true }).addTo(map);
    }

    // ---------- Info-paneel rechtsboven op de kaart
    const InfoCtl = L.Control.extend({
      options: { position: 'topright' },
      onAdd: function(){
        const el = L.DomUtil.create('div', 'infoctl');
        el.innerHTML = `
          <div class="title">Kaartinfo</div>
          <div class="row"><div class="k">FGR</div><div id="iF" class="v">—</div></div>
          <div class="row"><div class="k">Bodem</div><div id="iB" class="v">—</div></div>
          <div class="row"><div class="k">Gt</div><div id="iG" class="v">—</div></div>
          <div class="muted">Klik op de kaart om te updaten.</div>
        `;
        // voorkom dat scroll/drags hier de kaart bewegen
        L.DomEvent.disableClickPropagation(el);
        L.DomEvent.disableScrollPropagation(el);
        return el;
      }
    });
    const infoCtl = new InfoCtl();
    map.addControl(infoCtl);

    function setInfoBox(j){
      document.getElementById('iF').textContent = j.fgr || '—';
      const bodemTxt = (j.bodem || '—') + (j.bodem_bron ? ` (${j.bodem_bron})` : '');
      document.getElementById('iB').textContent = bodemTxt;
      document.getElementById('iG').textContent = (j.gt_code || '—') + (j.vocht ? ` → ${j.vocht}` : ' (onbekend)');
    }
    ...
    // Klik op kaart → advies + info-paneel updaten
    map.on('click', async (e)=>{
      ...
      setInfoBox(j);                    // >> info nu op de kaart i.p.v. onder de kaart
      renderRows(j.advies||[]);
