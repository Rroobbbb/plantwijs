...
def vocht_from_gwt(lat: float, lon: float) -> Tuple[Optional[str], dict, Optional[str]]:
    """Geef (klasse, ruwe_props, gt_code). Als niets te vinden is, klasse=None."""
    ...
    # Nieuw: probeer value_list (cm) indien aanwezig
    if not klass and props and "value_list" in props:
        try:
    ...
    # Fallback: GLG/GHG diepte (alleen als we daadwerkelijk een getal vinden)
    if not klass:
        for key in ("glg", "ghg"):
    ...
    # Als alles faalt → onbekend
    return None, props or {}, gt
