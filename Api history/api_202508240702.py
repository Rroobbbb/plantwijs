...
def bodem_from_bodemkaart(lat: float, lon: float) -> Tuple[Optional[str], dict]:
    ...
    for k in ("grondsoort", "bodem", "BODEM", "BODEMTYPE", "soil", "bodemtype", "SOILAREA_NAME", "NAAM"):
...
def advies_geo(
    ...
    bodem_val, props_bodem = bodem_from_bodemkaart(lat, lon)
    vocht_val, props_gwt, gt_code = vocht_from_gwt(lat, lon)
    bodem_val = bodem_val or "leem"
    vocht_val = vocht_val or "vochtig"
    ...
    out = {
        "fgr": fgr,
        "bodem": bodem_val,
        "bodem_bron": "BRO Bodemkaart WMS" if props_bodem else "fallback",
        "gt_code": gt_code,
        "vocht": vocht_val,
        "vocht_bron": "BRO Gt/GLG WMS" if props_gwt else "fallback",
        "advies": items,
