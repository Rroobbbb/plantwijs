r"""
PlantWijs API — v3.9.1 (Gt onbekend = géén filter)
===================================================
Wijzigingen t.o.v. v3.9:
- Fix: GT→vocht mapping vergeleek per ongeluk een string met een tuple.
- Nieuw: als GT/GLG/GHG niets opleveren is vocht **onbekend** en wordt géén
  vochtfilter toegepast (i.p.v. stilzwijgend defaulten naar een klasse).
- Klein: duidelijkere bronlabels (onbekend/fallback).
...
# ───────────────────── PDOK endpoints
HEADERS = {"User-Agent": "plantwijs/3.9.1"}
...
def _vochtklasse_from_gt_code(gt: Optional[str]) -> Optional[str]:
    if not gt:
        return None
    gt = gt.strip().upper()
    # FIX: membership i.p.v. tuple-equality
    if gt in ("I", "II"):
        return "zeer nat"
    if gt == "III":
        return "nat"
...
def vocht_from_gwt(lat: float, lon: float) -> Tuple[Optional[str], dict, Optional[str]]:
    """Haal GT op. Herken zowel Romeinse codes (I–VIII) als numerieke value_list (1–8 → I–VIII).
       Als alleen dieptes (cm) beschikbaar zijn via GLG/GHG, classificeer vocht op cm,
       maar laat gt_code leeg (we tonen dan nog steeds de vochtklasse).
    """
    ...
    # 1) Directe tekst/code uit properties (GT, Grondwatertrap, etc.)
    if props:
        for k in ("gt", "grondwatertrap", "GT", "Gt"):
            if k in props and props[k]:
                s = str(props[k]).strip().upper()
                # numeriek 1..8 → Romeins
                if s.isdigit():
                    n = int(float(s))
                    if 1 <= n <= 8:
                        s = roman_map[n]
        ...
        # 2) Numerieke 'value_list' (soms 1..8 als GT-code, soms cm — we onderscheiden beide)
        if gt_code is None and "value_list" in props and props["value_list"] not in (None, ""):
            s = str(props["value_list"]).strip().replace(",", ".")
            try:
                v = float(s)
                # exact integer 1..8 → GT-code
                if v.is_integer() and 1 <= int(v) <= 8:
                    gt_code = roman_map[int(v)]
                    klass = _vochtklasse_from_gt_code(gt_code)
                else:
                    # anders interpreteren als diepte (cm) en klassificeren op cm
                    cm = int(round(v))
                    if cm < 25:
                        klass = "zeer nat"
                    elif cm < 40:
        ...
        # 3) Fallback parsing uit platte tekst (GML/text/plain)
        if gt_code is None and "_text" in props:
            txt = str(props["_text"])
            # probeer sleutel:waarde
            kv = _parse_kv_text(txt)
            for k in ("gt", "grondwatertrap", "GT"):
                if k in kv and kv[k]:
                    s = str(kv[k]).strip().upper()
                    if s.isdigit():
                        n = int(float(s))
                        if 1 <= n <= 8:
                            s = roman_map[n]
                    ...
                    break
            if gt_code is None:
                # heuristiek: “GT  VII” of “GT: 6”
                m = re.search(r"\bGT\b[:=\s]*([IVX]+|\d{1})", txt, re.I)
                if m:
    ...
    # 4) Als we nog geen klass hebben, probeer GLG/GHG-diepte (cm)
    if klass is None:
        for key in ("glg", "ghg"):
            ...
            alltxt = " ".join(str(v) for v in p2.values())
            m = re.search(r"(GLG|GHG)\s*[:=]?\s*(\d{1,3})", alltxt, re.I)
...
# ───────────────────── app + cleaners
app = FastAPI(title="PlantWijs API v3.9.1")
...
def advies_geo(
    ...
    bodem_val = bodem_raw or "leem"  # bodem blijft zoals in v3.9 (alleen vocht wijzigt volgens wens)
...
<body>
  ...
  <script>
    ...
    map.on('click', async (e)=>{
      ...
      document.getElementById('ctxG').textContent = 'Gt: ' + (j.gt_code||'—') + (j.vocht?` → ${j.vocht}`:' (onbekend)');
      renderChips();
      renderRows(j.advies||[]);
