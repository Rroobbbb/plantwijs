...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    header { padding:10px 14px; border-bottom:1px solid var(--border); position:sticky; top:0; background:var(--bg); z-index:10; display:flex; align-items:center; gap:10px; }
    header h1 { margin:0; font-size:18px; }

    /* layout: exact 50/50 */
    .wrap { display:grid; grid-template-columns: 1fr 1fr; gap:12px; padding:12px; height:calc(100vh - 64px); }
    #map { height:100%; border-radius:12px; border:1px solid var(--border); box-shadow:0 0 0 1px rgba(255,255,255,.05) inset; overflow:hidden; }
    .panel { background:var(--panel); border:1px solid var(--border); border-radius:12px; padding:12px; }
    .panel-right { height:100%; overflow:auto; }

    /* filters boven de tabel, netjes uitgelijnd met de panel content */
    .filters { display:flex; flex-wrap:wrap; gap:8px 10px; margin:0 0 8px 0; align-items:center; }
    .filters .group { display:flex; gap:6px; align-items:center; flex-wrap:wrap; }
    .filters .lbl { color:var(--muted); margin-right:4px; }
    ...
    .chips { display:flex; gap:6px; flex-wrap:wrap; margin:6px 0 6px 0; }
    ...
    /* Locatieknop in de kaart (rechtsonder) */
    .locate-btn {
      background:#11325a; color:#e6edf3; border:1px solid #1f2c49; border-radius:999px;
      padding:8px; width:38px; height:38px; display:grid; place-items:center; cursor:pointer;
      box-shadow:0 2px 10px rgba(0,0,0,.4);
    }
    .locate-btn:hover { filter:brightness(1.08); }

    /* Leaflet layers control – titel erboven */
    .leaflet-control-layers { font:13px/1.4 system-ui,-apple-system,Segoe UI,Roboto,Arial; }
  </style>
</head>
<body>
  <header>
  ...
  <div class="wrap">
    ...
    <div class="panel panel-right">
      ...
      <div class="filters" id="filters">
        <div class="group">
          <label class="checks"><input id="inhOnly" type="checkbox" checked> alleen inheemse</label>
          <label class="checks"><input id="exInv" type="checkbox" checked> sluit invasieve uit</label>
        </div>

        <div class="group">
          <span class="lbl">Licht:</span>
          <label class="checks"><input type="checkbox" name="licht" value="schaduw"> schaduw</label>
          <label class="checks"><input type="checkbox" name="licht" value="halfschaduw"> halfschaduw</label>
          <label class="checks"><input type="checkbox" name="licht" value="zon"> zon</label>
        </div>

        <div class="group">
          <span class="lbl">Vocht:</span>
          <label class="checks"><input type="checkbox" name="vocht" value="zeer droog"> zeer droog</label>
          <label class="checks"><input type="checkbox" name="vocht" value="droog"> droog</label>
          <label class="checks"><input type="checkbox" name="vocht" value="vochtig"> vochtig</label>
          <label class="checks"><input type="checkbox" name="vocht" value="nat"> nat</label>
          <label class="checks"><input type="checkbox" name="vocht" value="zeer nat"> zeer nat</label>
        </div>

        <div class="group">
          <span class="lbl">Bodem:</span>
          <label class="checks"><input type="checkbox" name="bodem" value="zand"> zand</label>
          <label class="checks"><input type="checkbox" name="bodem" value="klei"> klei</label>
          <label class="checks"><input type="checkbox" name="bodem" value="leem"> leem</label>
          <label class="checks"><input type="checkbox" name="bodem" value="veen"> veen</label>
        </div>
      </div>
  ...
  <script>
    ...
    new InfoCtl({ position:'topright' }).addTo(map);
    ...
    // Locatieknop in de kaart
    const LocateCtl = L.Control.extend({
      onAdd: function() {
        const div = L.DomUtil.create('div');
        div.innerHTML = `
          <button class="locate-btn" title="Mijn locatie" aria-label="Mijn locatie">
            <!-- eenvoudige 'gps' icon -->
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
              <circle cx="12" cy="12" r="3" stroke="currentColor" stroke-width="2"/>
              <path d="M12 2v3M12 19v3M2 12h3M19 12h3M5.6 5.6l2.1 2.1M16.3 16.3l2.1 2.1M5.6 18.4l2.1-2.1M16.3 7.7l2.1-2.1" stroke="currentColor" stroke-width="2"/>
            </svg>
          </button>`;
        ...
        btn.onclick = ()=>{
          ...
          }, _=> alert('Kon locatie niet ophalen'));
        };
        L.DomEvent.disableClickPropagation(div);
        return div;
      }
    });
    new LocateCtl({ position:'bottomright' }).addTo(map);

    async function loadWms(){
      ui.meta = await (await fetch('/api/wms_meta')).json();
      const make = (m, opacity)=> L.tileLayer.wms(m.url, { layers: m.layer, format:'image/png', transparent: true, opacity, version:'1.3.0', crs: L.CRS.EPSG3857 });
      ...
      // Lagen-schakelaar linksonder (zonder base), met titel boven de lijst
    ...
    async function fetchList(){
      ...
      url.searchParams.set('limit','1000'); // alle geschikte tonen (ruime bovengrens)
    ...
    // Kaartklik → marker + context lezen + lijst ophalen met UI-filters + fallback kaartwaarden
