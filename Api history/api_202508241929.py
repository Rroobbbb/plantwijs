...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    /* Filters netjes boven de tabel */
    .filters { display:flex; flex-wrap:wrap; align-items:center; gap:10px 12px; padding:8px 10px; border-bottom:1px solid #182742; margin-bottom:8px; }
    .filters .group { display:flex; align-items:center; gap:8px; }
    .filters .lbl { color:var(--muted); margin-right:2px; }
    ...
    .chips { display:flex; gap:6px; flex-wrap:wrap; margin:8px 10px; }
    ...
    /* Locatieknop in kaart */
    .loc-btn { background:#0f192e; border:1px solid #1f2c49; width:34px; height:34px; border-radius:50%; display:grid; place-items:center; cursor:pointer; box-shadow:0 2px 8px rgba(0,0,0,.4); }
    .loc-btn:hover { background:#132644; }

    /* Leaflet layers control: titel bovenaan overlays */
...
<body>
  ...
  <div class="wrap">
    ...
    <div class="panel panel-right">
      <!-- Filters-balk boven de tabel -->
      <div id="filters" class="filters">
        <div class="group">
          <label class="checks"><input id="inhOnly" type="checkbox" checked> <span>alleen inheemse</span></label>
          <label class="checks"><input id="exInv" type="checkbox" checked> <span>sluit invasieve uit</span></label>
        </div>
      ...
      <div class="muted" id="count" style="padding:0 10px 8px;"></div>
  ...
  <script>
    ...
    // Locatieknop als Leaflet control (rechtsonder)
    const LocateCtl = L.Control.extend({
      onAdd: function(){
        const div = L.DomUtil.create('div');
        const btn = L.DomUtil.create('div', 'loc-btn', div);
    ...
    async function loadWms(){
      ...
      // Verwijder base-sectie + separator
      ...
      // Titel "Kaartlagen" boven de lijst
    ...
    async function fetchList(){
      ...
      // geen "(eerste N)" meer nodig → we vragen alles op
    ...
    async function refresh(){
      ...
      document.getElementById('count').textContent = data.count + ' resultaten';
    ...
    map.on('click', async (e)=>{
      ...
      document.getElementById('count').textContent = res.count + ' resultaten';
      renderRows(res.items || []);
    });

    // Listeners op alle inputs in de filters-balk
    for(const el of document.querySelectorAll('#filters input[type="checkbox"]')){
