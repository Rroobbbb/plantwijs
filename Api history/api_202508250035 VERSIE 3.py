# PlantWijs API — v3.9.6
# - UI: Kolomtitel opent filter (geen losse "Filter"-knop)
# - UI: Kolommen tonen/verbergen, sticky header, thema toggle, CSV/XLSX export
# - FIX: HTML triple-quoted string correct afgesloten
# Starten:
#   cd C:/PlantWijs
...
# ───────────────────── PDOK endpoints
HEADERS = {"User-Agent": "plantwijs/3.9.6"}
...
# ───────────────────── app + cleaners
app = FastAPI(title="PlantWijs API v3.9.6")
...
<body>
  ...
  <script>
    ...
    function uniqueTokensFor(items, key){
      ...
      for(const row of items||[]){
        if(key==='winterhardheidszone'){
          ...
          // Combineer bodem + grondsoorten voor filtering
    ...
    // Bouw de kolomkoppen; de TÍTELS zelf zijn de filter-triggers
    function buildTableHeader(){
      ...
      for(const c of COLS.filter(c=>c.visible)){
        ...
        lbl.textContent = c.label;   // ← klik op de titel opent het filter
