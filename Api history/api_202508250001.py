# PlantWijs API — v3.9.5
# - FIX: triple-quoted string error (HTML blok netjes afgesloten, geen module docstring meer)
# - UI: Kolommen tonen/verbergen + header multi-select filters
# - UI: Sticky header, thema toggle, CSV/XLSX export
# Starten:
#   cd C:/PlantWijs
...
# ───────────────────── PDOK endpoints
HEADERS = {"User-Agent": "plantwijs/3.9.5"}
...
# ───────────────────── app + cleaners
app = FastAPI(title="PlantWijs API v3.9.5")
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .th-filter { font-size:12px; border:1px solid var(--border); background:transparent; color:inherit; padding:2px 6px; border-radius:6px; cursor:pointer; }
    .th-filter:hover { background:rgba(255,255,255,.06); }
...
<body>
  ...
  <script>
    ...
    function buildTableHeader(){
      ...
      for(const c of COLS.filter(c=>c.visible)){
        ...
        if(c.filterable){
          ...
          btn.title = 'Filter op waarden';
          btn.addEventListener('click', (ev)=> openHeaderFilterMenu(ev.currentTarget, c.key, c.label));
    ...
    document.addEventListener('click', (e)=>{
      ...
      if(m1.classList.contains('show') && !m1.contains(e.target) && !(e.target.classList && e.target.classList.contains('th-filter'))){
