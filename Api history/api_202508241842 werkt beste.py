...
<body>
  ...
  <script>
   ...
   async function loadWms(){
    ...
    const make = (m, opacity)=> L.tileLayer.wms(m.url, {
      layers: m.layer, format:'image/png', transparent: true, opacity: opacity,
      version:'1.3.0', crs: L.CRS.EPSG3857
    });

    // Gebruik volledige titel voor FGR
    const titleFgr = (ui.meta.fgr && ui.meta.fgr.title) ? ui.meta.fgr.title : "Fysisch Geografische Regio's";
    ...
    overlays[titleFgr]                        = make(ui.meta.fgr,   0.45).addTo(map);
    ...
    // Verwijder eventuele base-sectie + separator
