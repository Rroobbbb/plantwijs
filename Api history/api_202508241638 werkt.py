...
<head>
  ...
  <style>
   ...
   /* Titel boven de lagen-control */
.pw-layers-title{
  color:#000;              /* echt zwart */
  font-weight:700;         /* duidelijk als titel */
  font-size:15px;          /* net iets groter dan de items */
  line-height:1.2;
  padding:6px 10px;
...
<body>
  ...
  <script>
    ...
    // Info-control (zonder legenden en zonder debug)
    ...
    async function loadWms(){
      ...
      // Lagen-control zonder OSM als baselaag + eigen titel "Kaartlagen"
      const layersCtl = L.control.layers({}, overlays, { collapsed:false }).addTo(map);
      const lc = layersCtl.getContainer();
      const title = document.createElement('div');
      title.className = 'pw-layers-title';
      title.textContent = 'Kaartlagen';
      lc.insertBefore(title, lc.firstChild);
    }
